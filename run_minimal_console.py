                ...
                if note.startswith(slot_name):
                    if "→" in note:
                        ...
                        if len(parts) == 2:
                            ...
                            trim_note = f", trimmed {original}→{final}"
                            break
...
def _apply_max_cap(shortlist: pd.DataFrame, k_auto_map: Optional[Dict[int, int]]) -> tuple[pd.DataFrame, List[str]]:
    ...
    for slot_name in SLOT_NAME_TO_ID.keys():
        ...
        notes.append(f"{slot_name}: trimmed {original_count}→{cap} (K-AUTO)")
...
def _format_daily_pnl_table(daily_data: Dict) -> List[str]:
    ...
    lines.append("💰 YESTERDAY'S P&L ({})".format(daily_data["date"].strftime('%d-%m-%y')))
...
def generate_detailed_daily_report(prediction_date: dt.date, cfg: PnLConfig) -> str:
    ...
    try:
        ...
        # Add notes
        report_lines.append("\n📝 NOTES")
        report_lines.append(f"• Daily report generated on {dt.datetime.now().strftime('%d-%m-%Y %H:%M')}")
...
def run_backtest_date_range(
    ...
    print_backtest_header("Date Range", 
                         f"Window: {start_date.strftime('%d-%m-%y')} → {end_date.strftime('%d-%m-%y')}")
    ...
    print(f"Backtest window: {start_date.strftime('%d-%m-%y')} → {end_date.strftime('%d-%m-%y')}")
    ...
    print(f"Total Return:   ₹{total_returns:.0f}")
    print(f"Total P&L:      ₹{total_pnl:+.0f}")
    print(f"Overall ROI:    {total_roi:+.1f}%")
    print(f"AB Stake:       ₹{total_ab_stake:.0f}")
    print(f"AB Return:      ₹{total_ab_returns:.0f}")
    print(f"AB P&L:         ₹{total_ab_pnl:+.0f}")
    print(f"AB ROI:         {total_ab_roi:+.1f}%")
    print(f"Total P&L:      ₹{total_combined_pnl:+.0f}")
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"Total Return: ₹{total_returns:.0f}\n")
    ...
    for day_num in range(total_days):
        ...
        try:
            ...
            if not shortlist.empty:
                ...
                date_folder = OUTPUT_DIR / prediction_date.strftime("%Y-%m-%d")
                print(f"\n✅ Predictions saved to: {date_folder}")
                
                processed += 1
            else:
                print(f"⚠️  No predictions generated for {prediction_date.strftime('%d-%m-%y')}")
...
def run_interactive_display(
    ...
    try:
        ...
        print("❌ Error: Shortlist not found")
    except Exception as e:
        print(f"❌ Error: {e}")
